...
bool tud_audio_set_itf_cb(uint8_t rhport, tusb_control_request_t const *p_request)
{
    ...
    if (ITF_NUM_AUDIO_STREAMING_SPK == itf && alt != 0) {
        ...
        ESP_LOGI(TAG, "Speaker interface %d-%d opened, bytes_per_ms=%zu, resolution=%d", 
                 itf, alt, s_uac_device->spk_bytes_per_ms, s_uac_device->spk_resolution);
    }
#endif
...
bool tud_audio_rx_done_post_read_cb(uint8_t rhport, uint16_t n_bytes_received, uint8_t func_id, uint8_t ep_out, uint8_t cur_alt_setting)
{
    ...
    if (now - last_time > 100 * CONFIG_UAC_SPK_NEW_PLAY_INTERVAL) {
        ...
        ESP_LOGI(TAG, "Audio new play detected, clearing FIFO");
    }
    last_time = now;
    ...
    if (new_play) {
        ...
        new_play = false;
        ESP_LOGI(TAG, "Audio buffering completed, starting playback");
    }
